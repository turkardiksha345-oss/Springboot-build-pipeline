...
pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
        git branch: 'main', credentialsId: 'GitlabCred', url: 'https://gitlab.com/Dikshaturkar10/springboot-build-pipeline.git'
      }
    }
   ...
   stage('Stage VII: Scan Image ') {
      steps { 
        ...
        sh "trivy image --scanners vuln --offline-scan dikshat1024/democicd:latest > trivyresults.txt"
        }
    }
   ...
   stage('Stage VIII: Smoke Test ') {
      steps { 
        ...
        sh "docker run -d --name smokerun -p 8080:8080 dikshat1024/democicd"
